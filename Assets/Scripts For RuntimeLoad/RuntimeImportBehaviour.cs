...
public class RuntimeImportBehaviour : MonoBehaviour
{
    ...
    private void OnComplete(GameObject importedModel)
    {
	    ...
	    _model.transform.parent = gameObject.transform;
	    _model.AddComponent<ResetObjectTransform>().Reset();
	    //_model.AddComponent<TouchRotationAndScale>();
    ...
    private void OnProgress(GltfImportStep step, int completed, int total)
    {
	    ...
	    SendMessage("SetProgress", completed/total, SendMessageOptions.RequireReceiver);
